...
Puppet::Type.type(:database_grant).provide(:mysql) do
  ...
  def create_row
    unless @resource.should(:privileges).empty?
      ...
      case name[:type]
      when :user
        mysql '--defaults-file=/root/.my.cnf', "mysql", "-e", "INSERT INTO user (host, user) VALUES ('%s', '%s')" % [
          name[:host], name[:user],
        ]
      when :db
        mysql '--defaults-file=/root/.my.cnf', "mysql", "-e", "INSERT INTO db (host, user, db) VALUES ('%s', '%s', '%s')" % [
  ...
  def row_exists?
    ...
    not mysql( '--defaults-file=/root/.my.cnf', "mysql", "-NBe", 'SELECT "1" FROM %s WHERE %s' % [ name[:type], fields.map do |f| "%s = '%s'" % [f, name[f]] end.join(' AND ')]).empty?
  end
  ...
  def privileges=(privs)
    ...
    mysql '--defaults-file=/root/.my.cnf', "mysql", "-Be", stmt
    mysql_flush
  end
end
